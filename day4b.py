...
def solve2(data):
    ...
    payload = data.replace(checksum, "").replace(sector_id, "").replace("-", " ").lower()
    output = ""
    for char in payload:

        if char != " ":
            char = chr((((ord(char) - 97) + int(sector_id)) % 26) + 97)
        ...
        for room in load_puzzle().split("\n"):
            if room.strip():
                ...
                if solve(room):
                    ...
                    if solve2(room) == "northpole object storage ":
                        print("Final answer", room)
